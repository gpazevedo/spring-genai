...
Our Terraform also manages a legacy group `/aws/bedrock-agentcore/runtimes/<runtime-name>` (without the `-DEFAULT` suffix) — this is unused and can be removed in a future cleanup.
...
- `xray:PutTraceSegments`, `xray:PutTelemetryRecords` — for trace delivery
